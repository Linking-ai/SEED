...
# Naive sampling algorithm, token level
def NaiveSampling(
...
# Speculative sampling algorithm, distribution level 
def SpeculativeSampling(
    ground_probs: torch.FloatTensor,
    cand_probs: Tuple[torch.FloatTensor],
    cand_tokens: torch.LongTensor,
) -> Optional[int]:
...
# Speculative sampling algorithm in MCSD method, distribution level
def MCSDSampling(
    ground_probs: torch.FloatTensor,
    cand_probs: Tuple[torch.FloatTensor],
    cand_tokens: torch.LongTensor,
...
# Base Strategy for different strategy used in speculative generator
class Strategy:
    def __init__(
        self,
        draft_model,
        ...
        if speculative_sampling:
            if replacement:
                self.acceptance_check = MCSDSampling
                ...
                self.acceptance_check = SpeculativeSampling
        else:
            if replacement:
                warnings.warn(
                    (
                        "`replacement` is not applicable when `speculative_sampling` is False."
                        "The acceptance check algorithm defaults to NaiveSampling"
                        " when `speculative_sampling=False`."
                    ),
                    category=UserWarning,
                    stacklevel=3,
                )
            self.acceptance_check = NaiveSampling
...
class BatchStrategy(Strategy):
    ...
    def generate_draft(
        ...
        for step in range(self.max_draft_len):
            ...
            if step + 1 != self.max_draft_len: # update kv
